...
- Stroke-order animation renders real glyph offscreen, reveals with a brush mask (`destination-in`); coords in `strokeOrder.js` are 0–100 space. Cleanup (`useEffect([letterIndex, formIndex, practiceMode])`) cancels RAF + clears animating state. `playStrokeAnimation` guards on `animatingRef.current`, NOT in `useCallback` deps.
  - **Coverage gate (ROADMAP #15):** `npm run test:stroke-coverage` runs `scripts/check-stroke-coverage.py` under a clean Store-Python env and asserts every _letter_ key in `strokeOrder.js` covers ≥95% of its Amiri ink (mask math mirrors `useAnimation.js`: `BRUSH_RADIUS = DOT_RADIUS = 0.2 * max(rendered w/h)`). Numerals/diacritics (quoted keys) are glyph logos, not guided strokes — excluded by design. Paths are hand-authored against a measured ink-run calibration table, **not** snapped to ink by algorithm (shape-agnostic snapping shortcuts through counters); see `canvas-reveal-animation-coverage` skill for the authoring recipe.
